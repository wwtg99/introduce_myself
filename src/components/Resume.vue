<template>
  <div class="resume">
    ...
    <div class="bio" id="bio">
      <p v-for="p in bio">{{ p }}</p>
    </div>
    <h2>Projects</h2>
    <div class="projects" id="projects">
      <Timeline>
        <TimelineItem v-for="t in projects">
          <p class="title">{{ t['title'] }} <span v-if="t['tags']" class="tags"><Tag v-for="tg in t['tags']">{{ tg }}</Tag></span></p>
          <p class="time">{{ t['time'] }}</p>
          <div class="content">
            <p v-for="p in t['descr']">{{ p }}</p>
